...
extern uint8_t raw_acc[6];
extern uint8_t raw_gyr[6];
...
//	lastUpdate = HAL_GetTick();
	...
	this->m_gyro_scale = c_MPU9250::E_GYRO_SCALE::GFS_2000DPS; 				// GFS_250DPS, GFS_500DPS, GFS_1000DPS, GFS_2000DPS
...
{
	  int16_t data[3];
	  this->f_read_bytes(MPU9250_ADDRESS, ACCEL_XOUT_H, 6, &raw_acc[0]);  // Read the six raw data registers into data array
	  data[0] = (int16_t)(((int16_t)raw_acc[0] << 8) | raw_acc[1]) ;  // Turn the MSB and LSB into a signed 16-bit value
	  data[1] = (int16_t)(((int16_t)raw_acc[2] << 8) | raw_acc[3]) ;
	  data[2] = (int16_t)(((int16_t)raw_acc[4] << 8) | raw_acc[5]) ;
...
{
	  int16_t data[3];
	  this->f_read_bytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6, &raw_gyr[0]);  // Read the six raw data registers sequentially into data array
	  data[0] = (int16_t)(((int16_t)raw_gyr[0] << 8) | raw_gyr[1]) ;  // Turn the MSB and LSB into a signed 16-bit value
	  data[1] = (int16_t)(((int16_t)raw_gyr[2] << 8) | raw_gyr[3]) ;
	  data[2] = (int16_t)(((int16_t)raw_gyr[4] << 8) | raw_gyr[5]) ;
	  ...
	  gyro.x -= this->m_gyro_bias[0];
	  gyro.y -= this->m_gyro_bias[1];
	  gyro.z -= this->m_gyro_bias[2];
}
...
void c_MPU9250::f_init_ak8963(void)
{
	  ...
	  myDelay(10);
	  this->f_read_bytes(AK8963_ADDRESS, AK8963_WHO_AM_I,1,&rawData[0]);
	  if(rawData[0] != 0x48)  // wait for magnetometer data ready bit to be set
	  {
		  while(1);
	  }
...
{
	gyro_data gyro_raw;
	for(int count_gyro = 0; count_gyro <= 1000; count_gyro++)
	{
//		this->read_gyro_data(gyro_raw);
		int16_t data[3];
		this->f_read_bytes(MPU9250_ADDRESS, GYRO_XOUT_H, 6, &raw_gyr[0]);  // Read the six raw data registers sequentially into data array
		data[0] = (int16_t)(((int16_t)raw_gyr[0] << 8) | raw_gyr[1]) ;  // Turn the MSB and LSB into a signed 16-bit value
		data[1] = (int16_t)(((int16_t)raw_gyr[2] << 8) | raw_gyr[3]) ;
		...
		// Para NED tengo que invertir x con y, ademas de negar z
		gyro_raw.x = (float)data[0] * this->f_get_gyro_res();  // get actual gyro value, this depends on scale being set
		gyro_raw.y = (float)data[1] * this->f_get_gyro_res();
		gyro_raw.z = (float)data[2] * this->f_get_gyro_res();


		if(count_gyro < 100)
		{
			this->m_gyro_bias[0] += gyro_raw.x;
			this->m_gyro_bias[1] += gyro_raw.y;
			this->m_gyro_bias[2] += gyro_raw.z;
		}
		else
		{
			this->m_gyro_bias[0] /= count_gyro;
			this->m_gyro_bias[1] /= count_gyro;
			this->m_gyro_bias[2] /= count_gyro;
		}
	}
