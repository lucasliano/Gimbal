...
#include <stdlib.h>
#include <stdint.h>
//#include "./inc/tm4c123gh6pm.h" 	//TODO:Ver este include!
#include "defines.h"
#include "MPU9250.h"
...
//Esto lo hacen para saber que printear mediante UART => Comentamos todo lo relacionado
/**
	#define DEBUG_PRINT
...
uint8_t MPU_init_data[16][2] = {
		...
		{0x00, MPUREG_SMPLRT_DIV},						 		// Set sample rate to 1 kHz																	//TODO: SAMPLE_RATE = Internal_Sample_Rate / (1 + SMPLRT_DIV)
		{0xCD, MPUREG_I2C_MST_CTRL},     						// I2C configuration multi-master  IIC 400KHz												//TODO:	Esto hay que sacarlo!
		{0x30, MPUREG_USER_CTRL},        						// Disable FIFO, enable I2C Master mode and set I2C_IF_DIS to disable slave mode I2C bus	//TODO: Esto hay que verlo bien
		{0x10, MPUREG_INT_PIN_CFG},								// Be cautious to change this.																//TODO:	Ver si hay que habilitar interrupt
...
//--------------TODO ESTO NO SIVER PARA NADA, ES ESPECÍFICO DE SPI--------------------------------------
/**
...
//TODO: Atención: Esto pareciera ser una implementación de SPI.. pensando que solo se comunica con el magnetómetro
//==================================================================================================================
static void bytewrite_AK8963(uint8_t address, uint8_t data) {
	// Data access from AK8963 (Magnetómetro) normally takes 1ms to 2ms to finish
	// Therefore don't use it too often

	const uint16_t microsec = 500;
	bytewrite_MPU(MPUREG_I2C_SLV0_ADDR, AK8963_I2C_ADDR);			// Write the Slave 0 AK8963 address and the direction of the data access
...
//TODO: Primitivas de lectura de Gyro, Accel y Magnetometro
//==================================================================================================================
void read_raw_accel(int16_t *buffer) {
	uint8_t temp_bytes[6];
	read_MPU(MPUREG_ACCEL_XOUT_H, temp_bytes, 6);					//Solo se intenta leer en la direc X
	buffer[0] = ((int16_t)temp_bytes[0] << 8)|temp_bytes[1];
	buffer[1] = ((int16_t)temp_bytes[2] << 8)|temp_bytes[3];		//TODO: ¿Por qué solo lee el MSB? ¿Qué son esas | que hace??
	buffer[2] = ((int16_t)temp_bytes[4] << 8)|temp_bytes[5];
}

void read_raw_gyro(int16_t *buffer) {
	uint8_t temp_bytes[6];
	read_MPU(MPUREG_GYRO_XOUT_H, temp_bytes, 6);					//Solo se intenta leer en la direc X
	buffer[0] = ((int16_t)temp_bytes[0] << 8)|temp_bytes[1];
...
static inline void read_raw_mag(int16_t *buffer) {
	...
	read_AK8963(AK8963_HXL, mag_rawdata, 7);						//Solo se intenta leer en la direc X
